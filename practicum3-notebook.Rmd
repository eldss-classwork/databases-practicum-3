...
journalDF <- function(num, node){
  print("journal")
  return(data.frame("article_id"=num))
}
```
...
grantDF <- function(num, node){
  ...
  for(i in 1:xmlSize(node)){
    newNode <- node[[i]]
    for(j in 1:xmlSize(newNode)){
      name <- xmlName(newNode[[j]])
      if(name == "GrantID"){
        id <- append(id, xmlValue(newNode))
      }
      else if(name == "Acronym"){
        ac <- append(ac, xmlValue(newNode))
      }
      else if(name == "Agency"){
        ag <- append(ag, as.numeric(xmlValue(newNode)))
      }
      else if(name == "Country"){
        ctry <- append(ctry, xmlValue(newNode))
      }
      else {
        print(paste("Vector not created for:", name))
      }
...
for(i in 1:xmlSize(root)){
  ...
  for(j in 1:xmlSize(newRoot)){
    ...
    if(name=="Article"){
      ...
      #   name <- xmlName(child)
      #   if(name=="ArticleDate"){
      #     print(paste("art date",xmlSize(child)))
      #   }
      # }
    }
  }
...
# Create an empty dataframe to match dataframes from above functions
df_journal <- data.frame("article_id"=c())
df_eLoc <- data.frame("article_id"=c(), "valid"=c(), "type"=c(), "value"=c())
df_abstract <- data.frame("article_id"=c())
df_author <- data.frame("article_id"=c())
...
```{r}
xpathSApply(article,"Abstract/CopyrightInformation", xmlValue)
```
