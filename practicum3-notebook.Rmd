...
Due to the small amount of data in this data-set (19 articles), we decided it would be more interesting to explore article activity, rather than just the dates they were published in journals. This includes all the different categories included in the `status` field. These are included below as a reference.

```{sql connection=conn}
SELECT * from PubStatus;
```
