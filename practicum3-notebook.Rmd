...
# Create an empty dataframe to match dataframe from eLoc function
...
for(i in 1:xmlSize(root)){
  ...
  for(j in 1:xmlSize(newRoot)){
    ...
    if(name=="Article"){
      id <- append(id,i)
      model <- append(model,xmlGetAttr(node,"PubModel",NA))
      article <- newRoot[[j]]
      for(k in 1:xmlSize(article)){
        ...
        if(name=="Journal"){
          ...
          aListC <- append(aListC,xmlGetAttr(node,"CompleteYN",NA))
          print("more needed")
        }
          ...
          gListC <- append(gListC,xmlGetAttr(node,"CompleteYN",NA))
          print("more needed")
        }
